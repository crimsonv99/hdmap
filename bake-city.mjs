...
import { initEngine, osmToLayers } from "./lib/build.mjs";
...
// NOTE: intersections get their `layer` stamped by a SEPARATE step, restamp-intersections.mjs,
// run after this (it joins each intersection's osm_node_ids to the shared node-altitude model
// so elevated bridge junctions get layer>0 and the viewer drops them at ground). Kept out of
// here so it can be re-run without the heavy per-tile osm2streets pass.
