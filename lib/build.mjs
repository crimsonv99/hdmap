...
// VENDORED into MVP-3D-MAP/lib so this repo builds standalone (no sibling repos).
// Source of truth: hdmap/build.mjs. If you change the road/building pipeline there,
// re-copy it here (and keep lib/elevation.mjs in sync). See README "Baking".
...
export async function initEngine() {
  if (ready) return;
  // resolve the wasm from the installed package (works wherever node_modules is)
  const wasm = readFileSync(require.resolve("osm2streets-js/osm2streets_js_bg.wasm"));
  await init(wasm);
  ready = true;
}
...
export function buildingsFromOsm(osmXml) {
  ...
  // sub-volumes exist, the outline is metadata only — the renderer draws the PARTS,
  // not the outline. Emitting both makes the outline's flat box double-render under
  // the detailed parts, so we drop any building=* outline that contains a part.
  // Gather the parts' centroids first (cheap, per-tile), then test each outline.
  const partPts = [];
  for (const [, w] of ways) {
    const bp = w.tags["building:part"];
    if (!bp || bp === "no") continue;
    let x = 0, y = 0, n = 0;
    for (const ref of w.refs) { const p = nodes.get(ref); if (p) { x += p[0]; y += p[1]; n++; } }
    if (n) partPts.push([x / n, y / n]);
  }
  const containsAPart = (ring) => {
    if (!partPts.length) return false;
    let minx = Infinity, miny = Infinity, maxx = -Infinity, maxy = -Infinity;
    for (const p of ring) { if (p[0] < minx) minx = p[0]; if (p[0] > maxx) maxx = p[0]; if (p[1] < miny) miny = p[1]; if (p[1] > maxy) maxy = p[1]; }
    for (const q of partPts) {
      if (q[0] < minx || q[0] > maxx || q[1] < miny || q[1] > maxy) continue; // bbox reject
  ...
  for (const [id, w] of ways) {
    ...
    // an outline (building=*, not itself a part) that contains parts is skipped — its
    // building:part sub-volumes render instead (S3DB §0). Parts always render.
    const isPart = w.tags["building:part"] && w.tags["building:part"] !== "no";
    if (!isPart && containsAPart(ring)) continue;
    features.push({
      type: "Feature",
